...
// [Authorize(Policy = "ModuleApp.view")] 
public class ModuleAppController : ControllerBase
{
    private readonly IModuleAppBusiness _repo;
    private readonly IStringLocalizerCustom _localizer;
    protected readonly ILogCustom _logger;

    public ModuleAppController(
           IModuleAppBusiness ModuleAppRepositoryBusiness,
         IStringLocalizerCustom localizer,
           ILogCustom logger)
    {
        _repo = ModuleAppRepositoryBusiness;
    ...
    [HttpPost("register")]
    // [Authorize(Roles = "hl-employee,hl-superadmin,hl-admin")]
    public async Task<IActionResult> Register(ModuleAppRegisterDto registerDto)
    {
    ...
    [HttpPost("Edit/{id}")]
    public async Task<IActionResult> Edit(int id, ModuleAppEditDto entityEdit)
    {
